...
def add_wypis():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS wypis
                 (id_w INTEGER PRIMARY KEY AUTOINCREMENT, 
                 pacjent INTEGER NOT NULL, 
                 lekarz INTEGER NOT NULL, 
                 data_przyjęcia DATE DEFAULT (datetime('now','localtime')) NOT NULL, 
                 data_wypisu DATE DEFAULT (datetime('now','localtime')) NOT NULL,
                 choroba TEXT NOT NULL)''')

    dane_wypis = [
        (1, 3, 3, '2022-12-20', '2023-01-15', 'dżuma'),
        (2, 4, 1, '2022-12-29', '2023-01-27', 'cholera'),
        (3, 15, 3, '2023-01-15', '2023-02-02', 'dżuma')
    ]


    for wypis in dane_wypis:
        c.execute(
            "INSERT INTO wypis (id_w, pacjent, lekarz, data_przyjęcia, data_wypisu, choroba) VALUES (?, ?, ?, ?, ?, ?)",
            wypis)
